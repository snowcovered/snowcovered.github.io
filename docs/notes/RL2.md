#  Improved RL for Learning to Branch (22)

[论文链接](https://arxiv.org/pdf/2201.06213)
...
* 奖励：自上一个状态以来对偶积分的**减少量**。  
  对偶积分定义为：
  $$
  F(T) = T \cdot c^\top x^* - \int_0^T z_t^* \, dt
  $$
  其中 $ c^\top x^* $ 为 MILP 的最优目标值（实践中用当前最佳可行解近似），$ z_t^* $ 为时刻 $ t $ 的全局对偶界。  即时奖励为：
  $$
  r(a_i|s_t) = -\big( F(t_{\text{curr}}) - F(t_{\text{prev}}) \big)
  $$
